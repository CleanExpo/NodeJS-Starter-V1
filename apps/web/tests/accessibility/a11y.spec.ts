...
test.describe('Accessibility Tests - PRD Generation', () => {
  ...
  test('form validation errors are announced to screen readers', async ({ page }) => {
    ...
    // Try to submit empty form. The form uses an accessible disabled-until-valid
    // submit (enabled only once the requirement is met), so only click when the
    // control is actually actionable — otherwise the empty-form state leaves it
    // disabled and clicking would hang.
