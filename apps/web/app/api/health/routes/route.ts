...
async function verifyRoute(route: RouteInfo, baseUrl: string): Promise<RouteInfo> {
...
export async function GET(request: NextRequest): Promise<NextResponse<RouteHealthResponse>> {
  ...
  if (verify) {
    routes = await Promise.all(routes.map((route) => verifyRoute(route, baseUrl)));
  }
