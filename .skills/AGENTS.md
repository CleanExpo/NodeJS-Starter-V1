...
10. **resilience-patterns** - For bulkhead isolation, timeout policies, and fallback chains
11. **rate-limiter** - For token bucket, sliding window, and tiered rate limiting
12. **data-validation** - For input sanitisation and schema patterns
13. **input-sanitisation** - For injection prevention and security
14. **rbac-patterns** - For role-based access control and permission hierarchies
15. **oauth-flow** - For OAuth 2.0/OIDC authorisation code flow with PKCE
16. **csrf-protection** - For CSRF prevention with double-submit cookies and origin checks
17. **secret-management** - For environment variable validation, redaction, and rotation
18. **content-moderation** - For content filtering, toxicity detection, and safety checks
19. **audit-trail** - For structured audit event logging and compliance
20. **structured-logging** - For observability and log patterns
21. **metrics-collector** - For metrics instrumentation and analytics data
22. **tracing-patterns** - For distributed tracing and span context propagation
23. **docker-patterns** - For multi-stage builds, layer caching, and Docker Compose
24. **ci-cd-patterns** - For GitHub Actions workflows, caching, and security scanning
25. **infrastructure-as-code** - For Terraform/Pulumi cloud provisioning and environment parity
26. **vector-search** - For pgvector embedding queries and similarity search
27. **search-indexer** - For full-text search indexing with tsvector and GIN indexes
28. **health-check** - For liveness, readiness, and dependency health probes
29. **graceful-shutdown** - For signal handling, connection draining, and clean teardown
30. **cache-strategy** - For caching patterns (lru_cache, Redis, Next.js fetch cache)
31. **api-contract** - For typed frontend/backend API contracts
32. **api-client** - For type-safe fetch wrappers with interceptors and retry
33. **api-versioning** - For URL/header-based versioning with deprecation policies
34. **graphql-patterns** - For GraphQL schema design, dataloader, and type-safe clients
35. **webhook-handler** - For idempotent webhook processing with signature verification
36. **state-machine** - For finite state machines and status transitions
37. **saga-pattern** - For distributed transaction orchestration with compensation
38. **pipeline-builder** - For composable data and task pipelines with typed stages
39. **feature-flag** - For feature toggles with percentage rollout and targeting
40. **workflow-engine** - For multi-step approval workflows with timeout and escalation
41. **queue-worker** - For Redis-backed background job processing
42. **notification-system** - For multi-channel notification dispatch with preferences
43. **slack-integration** - For Slack bot commands, webhook notifications, and team alerting
44. **cron-scheduler** - For scheduled tasks and periodic jobs
45. **csv-processor** - For CSV import, export, and streaming parse
46. **data-transform** - For ETL pipelines, typed mappers, and streaming transforms
47. **report-generator** - For multi-format report output and scheduled reporting
48. **markdown-processor** - For Markdown parsing, rendering, and MDX support
49. **pdf-generator** - For server-side PDF generation with React PDF and reportlab
50. **email-template** - For transactional email design and delivery
51. **changelog-generator** - For automated changelog and release notes from commits
52. **i18n-patterns** - For internationalisation with en-AU default locale
53. **genesis-orchestrator** - For workflow and phase management
54. **dashboard-patterns** - For real-time dashboard visualisation and layout
55. **status-page** - For public status page with incident management and uptime
56. **scientific-luxury** - For UI/design decisions
57. **xaem-theme-ui** - For theme generation and design token translation
58. **react-best-practices** - For React-specific optimisations
59. **web-design-guidelines** - For accessibility and UX audits
60. **playwright-browser** - For headless browser automation and E2E testing
61. **claude-browser** - For personal Chrome browser automation
